...
def draw_spec_set(spectrograms, maxrows=3, colsize=10, cmap=plt.cm.afmhot, zoom=2,fig_title='unknown',num_indv='unspecified'):
    """
    """
    ...
    ax.axis("off")
    ax.set_title(fig_title + ' Number of spec =' + str(num_indv))
    plt.show()
    
def draw_spec_set_actual(spectrograms, maxrows=3, colsize=10, cmap=plt.cm.afmhot, zoom=2):
    """
    """
    # get column and row sizes
    rowsize = np.shape(spectrograms[0])[0]
